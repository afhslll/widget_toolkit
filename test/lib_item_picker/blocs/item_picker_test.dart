...
void main() {
  ...
  group('test item_picker_bloc state', () {
    rxBlocTest<
      ItemPickerBlocType<ItemPickerDataMock>,
    ...
    >(
      ...
      expect: [Stubs.itemPickerLoading, Stubs.itemPickerDataSuccessResult],
    );
...
void _defineWhen() {
  when(
    service.getItems(),
  ).thenAnswer((_) => Future.value((Stubs.itemPickerData)));
}

ItemPickerBloc<ItemPickerDataMock> itemPickerBloc() =>
    ItemPickerBloc<ItemPickerDataMock>(service);
